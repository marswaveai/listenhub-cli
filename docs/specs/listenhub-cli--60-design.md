...
**目标**：`vp pack --platform node` 以 `source/cli.ts` 为入口，bundle 成单文件 `dist/cli.mjs`。
...
**现状**：CLI 没有单元测试，`npm test` 只是跑 xo lint。
...
- `xo`
- `typescript`（vp 内置）
- `@sindresorhus/tsconfig`——SDK 同样 extends 了这个 preset 且与 vp check 兼容。如果 CLI 迁移后 `vp check` 报错再考虑移除，默认保留
...
3. `pnpm lint` 和 `pnpm fmt:check` 通过
4. `pnpm check` 类型检查通过
